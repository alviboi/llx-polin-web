...
class polinPacMan {
    ...
    drawPackageList() {
        ...
        for (var i in self.packageList) {
            ...
            var cols = [self.packageList[i].appname, self.packageList[i].flavour, self.packageList[i].lastUpdate, self.packageList[i].watch_me, self.packageList[i].description, self.packageList[i].lliurexversion, self.packageList[i].upstreamversion, self.packageList[i].translatedVersions];
            console.log(cols);
            var row = $(document.createElement("tr"));
            var col ="";
            ...
            for (col in cols) {
                ...
                if (col === "5" && cols[5] !== cols[6])
                    $(row).addClass("updateable");
                if (col === "5" && cols[5] > cols[7].split(" ").join(""))
                    $(row).addClass("updateableTrans");
                if (col === "0")
                    $(td).addClass("poLink").attr("filename", self.packageList[i].filename);
                if (col === "3")
                    if (self.packageList[i].watch_me == "true")
                        $(td).html('<span class="glyphicon glyphicon-ok"></span>').css("text-align", "center");
                    else
                        $(td).html('');
                $(row).append(td);
            }
            //} else {
            //console.log("Algo pasa cols");
    ...
    addNewApp() {
        ...
        if (filename)
            self.showConfigDialog(pkgInfo, filename + ".json", defaultDist);
    }
    delNewApp(filename) {////////////////////////////////////////////////////////////////////////////
            ...
            buttons: {
                "Esborra": {
                    ...
                    callback: function () {
                        ...
                        self.delPkgInfo(filename, defaultDist);
                        location.reload();
                    }
                }
            }
    ...
    delPkgInfo(filename, defaultDist){
        var data = { action: "delPackage", dist: dist, filename: filename + ".json"};
        $.ajax({
            type: 'POST',
            url: "controller.php",
            data: data,
            ...
            success: function (ret) {
                if (ret.status) {
                    ...
                    $("#messages").empty();
                    var item = $(document.createElement("div")).addClass("successmsg").html("El fitxer " + dist + "/" + filename + " s'ha esborrat correctament.");
    ...
    showConfigDialog(pkgInfo, filename, defaultDist) {
        var self = this;
        var content = "";
        console.log(pkgInfo);
        ...
        for (item in checkArray) {
            if (pkgInfo[checkArray[item]] === undefined)
                pkgInfo[checkArray[item]] = "";
        }
        /*  App Name */
        content += self.formFactory.createText({
        "id": "appname",
            "label": "Nom de l'aplicació",
            "help": "Nom de l'aplicació (no necessàriament del paquet)",
            "value": pkgInfo.appname
        }, 2);
        /*  Flavour */
        content += self.formFactory.createText({
        "id": "flavour",
        ...
        /* last update */
        content += self.formFactory.createDate({
        "id": "lastUpdate",
            "label": "Última actualització",
            "help": "Data de la última actualització",
            "value": pkgInfo.lastUpdate
        }, 4);
        /* Requires attention (watch me)*/
        var req = "";
        if (pkgInfo.watch_me == "true")
            req = "checked";
        content += self.formFactory.createCheckbox({
        "id": "watch_me",
            "label": "Req. Atenció",
            "default": req,
            "help": "Indica si la traducció requereix d'alguna atenció especial (reviseu anotacions)"
        }, 4);
        /* Versions */
        content += self.formFactory.createText({
        "id": "lliurexVersion",
        ...
        content += self.formFactory.createTextArea({
        "id": "packagelist",
            "label": "Paquets",
        ...
        var versions = "";
        for (var ver in pkgInfo.translatedVersions) {
        ...
        "id": "translationType",
            ...
            "options": [
                ...
                { "value": "other", "label": "Altres mecanismes" }
            ]
        }, 2);
        /*Traslation Method*/
        content += self.formFactory.createSelect({
