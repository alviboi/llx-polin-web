...
formFactory.prototype.createDate=function createText(item, columns=1){
  ...
  var sel='<div class="form-group '+cols_str+'" title="'+item.help+'" controlid="'+item.id+'">';
    sel+='<label for="'+item.id+'" class="'+left_col_size+' control-label">'+item.label+'</label>';
    sel+='<div class="'+right_col_size+'">';
    sel+='<input id="'+item.id+'" class="form-control" type="date" value="'+item.value+'" '+disabled+' style="line-height:20px">';
    sel+="</input></div></div>";
    return sel;
};
